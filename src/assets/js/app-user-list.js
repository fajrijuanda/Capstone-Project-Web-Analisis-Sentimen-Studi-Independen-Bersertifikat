...
$(function () {
  ...
  if (dt_user_table.length) {
    var dt_user = dt_user_table.DataTable({
      ajax: '/users/list/',
      columns: [
        // columns according to table accounts_profile
        { data: 'id' },
        { data: 'id' },
        { data: 'full_name' },
        { data: 'role' },
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            var $name = full['full_name'],
              $email = full['email'],
              $image = full['avatar']; // Perbaiki di sini, gunakan URL avatar langsung dari server.
            if ($image) {
              // For Avatar image
              var $output = '<img src="' + $image + '" alt="Avatar" class="rounded-circle" width="40" height="40">';
            } else {
              // For Avatar badge (jika avatar tidak tersedia)
